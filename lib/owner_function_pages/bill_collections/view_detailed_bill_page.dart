...
class ViewDetailedBillPage extends StatefulWidget {
  Bill bill;
  String date;
  ViewDetailedBillPage({this.bill,this.date});

  @override
  _ViewDetailedBillPageState createState() => _ViewDetailedBillPageState();
}

class _ViewDetailedBillPageState extends State<ViewDetailedBillPage> {
                  ...
                  child: Column(
                    children: <Widget>[
                      Row(
                        ...
                        children: <Widget>[Text('${chosenBill.fname} ${chosenBill.lname}'.toUpperCase(),style: smallTextStyle(color: ThemeColor.themeBlueColor),)],),
                  ...
                  KiButton.rectButton(
                    onPressed: ()async{
                      if(chosenBill.status==0){
                        ...
                        Fluttertoast.showToast(msg: 'Only unpaid bill can be resend',
                    ...
                    child: Text('RESEND',style: extraSmallTextStyle(color: ThemeColor.themeBlueColor),)
                  ),
                  ...
                  KiButton.rectButton(
                    onPressed: ()async{
                      try{
                        ...
                        int status = chosenBill.status==0?1:0;
                        await kindergarten.doc('${chosenBill.kindergarten}/bills/${formatter.format(DateTime.now())}/${formatter.format(DateTime.now())}/${chosenBill.uid}').update({
                          'status':status,
                          'paidTime':now,
                        });
                        await guardian.doc('${chosenBill.uid}/bills/${formatter.format(DateTime.now())}/${formatter.format(DateTime.now())}/${chosenBill.kindergarten}').update({
                          'status':status,
                          'paidTime':now,
                        });

                        DocumentSnapshot document =await kindergarten.doc('${chosenBill.kindergarten}/bills/${formatter.format(DateTime.now())}').get();
                        Map data =document.data();
                        if(status==0) {
                          await kindergarten.doc('${chosenBill
                              .kindergarten}/bills/${formatter.format(
                              DateTime.now())}').update({
                            'totalUnPaid': data['totalUnPaid']+chosenBill.totalFee,
                            'totalPaid': data['totalPaid']-chosenBill.totalFee,
                            'totalUnPaidNo': data['totalUnPaidNo']+1,
                            'totalUnPaidStudents': data['totalUnPaidStudents']+chosenBill.noOfBills,
                            'totalPaidStudents': data['totalPaidStudents']-chosenBill.noOfBills,
                            'totalPaidNo': data['totalPaidNo']-1,
                            'status': (data['totalUnPaid']-chosenBill.totalFee)==0?1:0,
                            'updateTime':now
                          });
                        }
                        else{
                        ...
                        Fluttertoast.showToast(msg: 'Bill status is updated',
                            backgroundColor: ThemeColor.themeBlueColor,
                            textColor: ThemeColor.whiteColor,
                            fontSize: SizeConfig.smaller);
                      }catch(e){
                    ...
                    child: Text(chosenBill.status==0?'PAY':'UNPAID',style: extraSmallTextStyle(color: ThemeColor.themeBlueColor),)
                  )
                ],
              )
            ],
