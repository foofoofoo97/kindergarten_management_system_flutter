...
class ViewStudentAttendancePage extends StatefulWidget {
  int index;
  ViewStudentAttendancePage({this.index});
  @override
  _ViewStudentAttendancePageState createState() => _ViewStudentAttendancePageState();
}
...
class _ViewStudentAttendancePageState extends State<ViewStudentAttendancePage> {
  ...
  Future<void> prepareList()async{
    QuerySnapshot querySnapshot= await student.doc(kindergartenProfile.studentUID[widget.index]).collection('attendance').orderBy('datetime',descending: true).get();
              ...
              child: Column(
                  ...
                  children: <Widget>[
                    Row(
                        ...
                        children: <Widget>[
                            Text(
                              '${kindergartenProfile.studentFirstName[widget.index]} ${kindergartenProfile.studentLastName[widget.index]}',
                      ...
                      children: <Widget>[
                        ...
                        IconButton(
                          ...
                          onPressed: ()async{
                            ...
                            QuerySnapshot querySnapshot= await student.doc(kindergartenProfile.studentUID[widget.index]).collection('attendance').where('datetime',isGreaterThan: start).where('datetime',isLessThan: end).orderBy('datetime',descending: true).get();
                            for(DocumentSnapshot documentSnapshot in querySnapshot.docs){
                              Map data =documentSnapshot.data();
                              Attendance attendance = new Attendance(
                                  dateTime: data['datetime'].toDate(),
  ...
  Widget checkInWidget2(){
    return Row(
      children: <Widget>[
        ...
        SizeConfig.mediumHorizontalBox,
        Text('Check In Is Done By Owner/ Staff', style: smallererTextStyle(color: ThemeColor.blackColor
        ),)
      ],
    );
  }
